...
def write_hu_moments(label, writer):
    files = glob.glob('machine/shapes/' + label + '/*')  # label recibe el nombre de la carpeta
    hu_moments = []
    for file in files:
...
def generate_hu_moments_file():
    with open('machine/generated-files/shapes-hu-moments.csv', 'w',
...
def hu_moments_of_contour(contour):
    # Calculate Moments
    moments = cv2.moments(contour)  # momentos de inercia
    # Calculate Hu Moments
    huMoments = cv2.HuMoments(moments)  # momentos de Hu
    # Log scale hu moments
    for i in range(0, 7):
